...
public class OnlineResourceBeanManager extends BaseCloseMBean
{
	...
	void LoadResourceCache(boolean bCacheResourceFiles)
	{
		...
		// 1st load the .res files
		File lst[] = FileSystem.getFileListBySuffix(m_resourceManager.m_csResourcePath, ".res");
		if (lst != null)
		{
			Log.logImportant("Beginning to cache " + lst.length + " resources files from " + m_resourceManager.m_csResourcePath);
		...
		Log.logNormal("LoadResourceCache Unique XML files loaded=" + lst.length + "; XML from jar resource=" + nXMLResource + "; Total load Time (ms)=" + sw.getElapsedTime());
